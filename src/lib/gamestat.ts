...
import type { GameStat } from '$lib/GameStats';
...
export async function addGameStat(gamestat: GameStat) {
  ...
  let extrapass = gamestat.extrapass;
  let extrafail = gamestat.extrafail;
  let faceoffwin = gamestat.faceoffwin;
  let faceoffloss = gamestat.faceoffloss;
  const result = await sql`
    INSERT INTO gamestats (sheetid, side, quarter, ground, shots, clearpass, clearfail, extrapass, extrafail, faceoffwin, faceoffloss) VALUES (${sheetID}, ${side}, ${quarter}, ${ground}, ${shots}, ${clearpass}, ${clearfail}, ${extrapass}, ${extrafail}, ${faceoffwin}, ${faceoffloss}) RETURNING *
  `
